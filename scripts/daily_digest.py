...
def tag_paper(title, abstract, api_key):
    ...
    if not abstract:
        return []
    client = Groq(api_key=api_key)
    keys = ", ".join(f'"{k}"' for k in TAG_CATEGORIES)
    prompt = (
        f"Classify this paper into 1-2 categories. "
        f"Valid keys: {keys}\n"
        f"Title: {title[:150]}\n"
        f"Abstract: {abstract[:400]}\n"
        f"Reply with ONLY a JSON array, e.g. [\"foundation\"] or [\"vision\",\"efficient\"]."
    )
    ...
    for attempt in range(3):
        try:
            resp = client.chat.completions.create(
                model=TAG_MODEL,
                messages=[
                    {"role": "system", "content": "You output only a JSON array of category keys. No explanation."},
            ...
            if not m:
                print(f"    태깅 파싱 실패 (no array). 원문: {text!r}")
                return []
            tags = json.loads(m.group(0))
            result = [t for t in tags if isinstance(t, str) and t in VALID_TAGS][:2]
            if not result:
                print(f"    태깅 유효 키 없음. 원문: {text!r}")
            return result
        except Exception as e:
            err = str(e)
            if "429" in err or "rate" in err.lower():
                ...
                time.sleep(wait)
                continue
            print(f"    태깅 실패 (attempt {attempt+1}): {e}")
            if attempt == 2:
                return []
    return []
...
def save_to_supabase(papers_data, date_str):
    ...
    # tagged_at을 명시적으로 포함해 upsert 시 오늘 날짜로 갱신 — 이전 달 tagged_at이 남아
    # 이번 달 월간 집계 쿼리에서 누락되는 것을 방지한다.
